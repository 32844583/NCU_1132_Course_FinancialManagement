...
def get_text_overlapping_chunk(
    ...
    * Returns a dictionary where the keys are chunk numbers and the values are the corresponding text chunks.
    ...
    for i in range(0, len(text), character_limit - overlap):
        ...
        chunked_text_dict[chunk_number] = chunk # 直接賦值
...
def get_image_for_gemini(
    ...
    # --- 修正 2: 使用 os.path.join 來建立跨平台相容的路徑 ---
    # 組合出最終的檔名
    output_filename = f"{base_file_name}_image_{page_num}_{image_no}_{xref}.png"
    # 使用 os.path.join 安全地組合出完整的儲存路徑
    image_save_path = os.path.join(image_save_dir, output_filename)
    ...
    # 從正確的路徑載入圖片
    image_for_gemini = Image.load_from_file(image_save_path)
    
    return image_for_gemini, image_save_path
